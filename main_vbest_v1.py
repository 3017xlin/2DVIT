"""V_best_v1 main: V_best with smaller PointNet radii.

Change vs V_best:
  pointnet_scales = ((0.15, 32), (0.5, 64))    # V_best default
  pointnet_scales = ((0.08, 32), (0.20, 48))   # V_best_v1

Hypothesis: ViT covers global mixing, so encoder doesn't need large radius;
encoder should focus on local (BL-scale) geometry only.
"""
...
for i in range(args.nmodel):
    ...
    model = UrbanWindViT(
        ...
        pointnet_scales=((0.08, 32), (0.20, 48)),   # V_best_v1: smaller radii
        latent_dim=256,
        patch_size=2,
        num_layers=5,
        num_heads=8,
