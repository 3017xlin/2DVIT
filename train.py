...
def main(device, train_dataset, val_dataset, Net, hparams, path, criterion='MSE', reg=1, val_iter=10,
    ...
    model = Net.to(device)
    # JIT-compile for kernel fusion (~15-25% speedup). mode='default' has
    # manageable compile cost (~1-2 min on first epoch). dynamic=True avoids
    # recompilation when airfoil_pos varies in size between cases.
    # Set NO_COMPILE=1 to skip if anything breaks at compile time.
    if torch.cuda.is_available() and os.environ.get('NO_COMPILE') != '1':
        try:
            ...
            if is_main:
                print('torch.compile failed, continuing without compile: {}'.format(e))
    if is_distributed:
    ...
    # Steps per rank per epoch must account for DistributedSampler splitting the data
    # across ranks; otherwise OneCycleLR's peak ends up at ~50% of training instead
    # of pct_start=0.3, leaving the model in late warmup well past the intended peak.
    effective_world_size = world_size if is_distributed else 1
    steps_per_epoch = len(train_dataset) // (hparams['batch_size'] * effective_world_size) + 1
