"""V14 main: V8 + auxiliary (Cd, Cl) supervision head.

Single change vs V8: model is UrbanWindViT_v14 (returns tuple),
training is train_v14 (handles tuple output and adds Cd/Cl aux loss).
Dataset and loss weights are unchanged from V8.

Note: eval_score / test_field_mse must be aware that the model returns
a tuple (field, coef_aux). Use test_field_mse_v14.py and eval_score_v14.py
on HPC (provided as helper scripts).
"""
import argparse, yaml, json, os
import torch
import train_v14 as train
...
for i in range(args.nmodel):
    if args.model == 'UrbanWindViT':
        from models.UrbanWindViT_v14 import UrbanWindViT
        model = UrbanWindViT(
            grid_size=64,
        ...
        raise ValueError(f"main_v14 only supports UrbanWindViT, got {args.model}")
