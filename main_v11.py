"""V11 main: V8 + log-space nut target.

Single change vs V8: import dataset.dataset_v11.Dataset (which applies
log to nut before normalization), and import models.UrbanWindViT_v11
(structurally identical to V8 but named V11 for clarity).

train_v11 differs from train_v8 only in per-variable loss weights
(nut weight restored to 1.0 since log already balances dynamic range).
"""
import argparse, yaml, json, os
import torch
import train_v11 as train  # V11: AdamW + balanced loss weights + smart schedule
import utils.metrics as metrics
from dataset.dataset_v11 import Dataset
import os.path as osp
import numpy as np

parser = argparse.ArgumentParser()
parser.add_argument('--model', help='The model you want to train', type=str)
...
for i in range(args.nmodel):
    if args.model == 'UrbanWindViT':
        from models.UrbanWindViT_v11 import UrbanWindViT
        model = UrbanWindViT(
            grid_size=64,
        ...
        raise ValueError(f"main_v11 only supports UrbanWindViT, got {args.model}")
