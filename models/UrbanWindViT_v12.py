"""V12 model: V8 + per-point exact wall-gradient as decoder identity feature.

Single change vs V8: data.x is now [N, 9] with the last 2 columns being
the exact unit vector from each point toward the nearest airfoil-surface
point (computed in dataset_v12 via cKDTree). The decoder concatenates
those 2 channels into the identity feature so the prediction head sees
the precise wall-normal direction at every query point — independent of
the 64x64 grid resolution that limits sdf_grad_at_query in V8.

Encoder is unchanged (still uses uinf, sdf, sdf_grad on the grid).
"""
...
class DecoderV12(nn.Module):
    """V12 decoder: V8 decoder + 2-channel exact wall-gradient in identity."""
    ...
    def __init__(
        ...
        # V8 had identity_dim = 9 (pos[2] + uinf[2] + sdf[1] + sdf_grad_query[2] + normals[2]).
        # V12 adds wall_grad[2] -> identity_dim = 11.
        identity_dim = 11
    ...
    def physical_to_norm(self, pos):
        ...
        if self.training and not DecoderV12._oob_warned:
            with torch.no_grad():
    ...
    def forward(self, processed_grid, sdf_grad_grid, query_pos,
                query_uinf, query_sdf, query_normals, query_wall_grad):
        N = query_pos.shape[0]
...
class UrbanWindViT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.__name__ = 'UrbanWindViT_v12'
    ...
    def forward(self, data):
        ...
        out = self.decoder(
            ...
            x[:, 4:5],   # sdf
            x[:, 5:7],   # normals
            x[:, 7:9],   # V12 NEW: exact per-point wall-grad
        )
        return out
