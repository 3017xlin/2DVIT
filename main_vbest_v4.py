"""V_best_v4 main: V_best + deeper decoder (pos_mlp/pred_head 2->4 layers).

Change vs V_best: model is UrbanWindViT_vbest_v4 with Decoder_Deep. The
pos_mlp and pred_head each grow from 2 linear layers to 4 (with ReLU in
between), hidden width unchanged. Adds ~265K parameters.
"""
...
for i in range(args.nmodel):
    from models.UrbanWindViT_vbest_v4 import UrbanWindViT
    model = UrbanWindViT(
        grid_size=64,
