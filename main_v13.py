"""V13 main: V8 + (log10 Re, AoA) explicit injection.

Single change vs V8: import dataset.dataset_v13.Dataset (broadcasts flow
features per point) and import models.UrbanWindViT_v13 (decoder identity
has 11 channels). Loss weights and training loop unchanged.
"""
import argparse, yaml, json, os
import torch
import train_v8 as train  # V13 reuses V8 training (no loss change)
import utils.metrics as metrics
from dataset.dataset_v13 import Dataset
...
for i in range(args.nmodel):
    if args.model == 'UrbanWindViT':
        ...
        raise ValueError(f"main_v13 only supports UrbanWindViT, got {args.model}")
